...
import {
  ...
  FormFeedback, Button, Form, FormGroup, Label, Input, Col
} from 'reactstrap';
...
class Contact extends Component {
  ...
  validate(firstname, lastname, email, telnum) {
    ...
    if (this.state.touched.lastname && lastname.length < 3)
      errors.lastname = 'Lastname Name should be >= 3';
    else if (this.state.touched.lastname && lastname.length > 10)
      errors.lastname = 'Lastname Name should be <= 10';

    const regex = /^\d+$/;
    if (this.state.touched.telnum && !regex.test(telnum))
      errors.telnum = 'Lastname should only contain numbers';

    if (this.state.touched.email && email.split('').filter((char) => char === '@').length !== 1)
      errors.email = 'email should only contain a @ sign';
